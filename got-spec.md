...
|--------------|----------------------------------------------------------|
| Language     | Go 1.22+                                                 |
| Go module    | `github.com/got-sh/got` (replaceable via `find`/`sed` at scaffold time) |
...
- **Module path `github.com/got-sh/got`** is a working default. The `got-sh/` prefix is generic enough to remain valid if the project moves to its own org later, and the whole path is one `find ... | xargs sed` away from being swapped to `github.com/<real-org>/got` and `got-sh/homebrew-tap` → `<real-org>/homebrew-tap`. The CI workflows and `.goreleaser.yml` use the same path so a single substitution updates everything.
...
builds:
  - main: ./cmd/got
    ...
    ldflags:
      ...
      - -X github.com/got-sh/got/internal/version.Commit={{.ShortCommit}}
brews:
  - repository:
      owner: got-sh
      name: homebrew-tap
    homepage: https://github.com/got-sh/got
...
`got-sh/got` in the repo and fails CI if any stray reference remains.
...
- `README.md`: install (`brew install got-sh/homebrew-tap/got`),
...
1. **Module path / Homebrew tap** — *Resolved.* Go module is
   `github.com/got-sh/got`; Homebrew tap is `got-sh/homebrew-tap`; the
   GitHub repo URL is `https://github.com/got-sh/got`. See §3 and §18.
...
- [ ] `make check-paths` confirms no stray `<org>` placeholders remain
       in code, YAML, or shell scripts (locks in the `got-sh/got` module
...
1. `go.mod` (module `github.com/got-sh/got`), `LICENSE` (MIT),
